...
function confLabel(conf) {
  return { high:'Alta', medium:'Media', low:'Baja' }[conf] || '';
}
...
function pickCard(m) {
  ...
  <div class="pick-card sport-${sport}">
    <div class="pick-top">
      <span class="chip ${sport}">${league}</span>
    ...
    <div class="pick-match">
      ...
      <div class="pick-vs">${m.score || 'vs'}</div>
...
function renderBets() {
  ...
  if (showForm) html += `
    <div class="add-form">
      ...
      <div class="field"><label>Partido</label><input id="f-m" type="text" placeholder="Ej: Arsenal vs Burnley"></div>
      <div class="field"><label>Pick</label><input id="f-p" type="text" placeholder="Ej: Local, Ambas marcan"></div>
      <div class="field-row">
        <div class="field"><label>Cuota</label><input id="f-o" type="number" step="0.01" placeholder="1.85"></div>
        <div class="field"><label>Stake $</label><input id="f-s" type="number" placeholder="100"></div>
      </div>
      <div class="field"><label>Liga</label>
        <select id="f-l">
...
function analyze(id) {
  ...
  alert(`Para análisis IA abre Sports Peecks desde el chat de Claude.\n\n${m.home} vs ${m.away} — ${liga}${series}`);
}
